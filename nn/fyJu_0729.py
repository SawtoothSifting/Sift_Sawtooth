...
# find var in 2nd floor+ stack mem
    ...
    def update_num_select(epoch, batch, l, gidx, xidx):
        ...
        return False
    def binpack01(m,idxl:list):
        ans=[]
        binmatx = [0] * (m + 1)
        for idx in idxl:
            for i in range(len(idxl)):
                for j in range(m, hist[l][idx][2] - 1, -1):
                    if binmatx[j]<=binmatx[j - hist[l][idx][2]] + hist[l][idx][2]:
                        binmatx[j] =binmatx[j - hist[l][idx][2]] + hist[l][idx][2]
                        if ans.empty!=True:
                            ans=ans[idxl[j]]
                        ans.append(idx)
        return ans
                    ...
                    for xidx, gidx in enumerate(block._node_frames[1]['_ID']):
                        ...
                        try:
                            if cut_num_select(epoch, batch, l, gidx, xidx):
                                x[xidx].data = hist[l][gidx.item()].to(
                                    device).data
                                # print(gidx)
                        except KeyError:
                            pass
